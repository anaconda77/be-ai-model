...
try:
    ...
    raise exceptions.ModelLoadError(f"FinBERT 모델 로딩에 실패했습니다. (모델명: {model_name}): {e}")
...
def analyze_sentiment_with_progress(df: pd.DataFrame, batch_size: int = 32) -> pd.DataFrame:
    try: # [추가] 감성 분석 전체 과정을 try-except로 감쌉니다.
        ...
        raise exceptions.SentimentAnalysisError(f"FinBERT 파이프라인 실행 중 오류 발생: {e}") from e
...
def update_sentiment_score_in_db(supabase_client, df_to_update):
    ...
    if 'news_id' not in df_to_update.columns or 'sentiment_score' not in df_to_update.columns:
        ...
        raise ValueError("업데이트를 위해 DataFrame에 'news_id'와 'sentiment_score' 컬럼이 필요합니다.")
